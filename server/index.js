...
app.get("/page", (req, res) => {
  ...
  const commonsDebug = debug_comp
    ? [`${CONFIG.HOST}:${CONFIG.DEV_SERVER_PORT}/commons-dev.js`]
    : [];
  const {
    page: { title, keys, desc },
  } = getPageJSON();
  ...
  res.render(path.join(__dirname, "./template/index.ejs"), {
    ...
    desc,
    js: [
      `${CONFIG.DOMAIN}/page/commons.js`,
      `${CONFIG.DOMAIN}/page/main.js`,
      ...commonsDebug,
    ],
  });
});
...
app.get("/edit", (req, res) => {
  ...
  res.render(path.join(__dirname, "./template/index.ejs"), {
    ...
    js: debug
      ...
      : [
          `${CONFIG.DOMAIN}/edit/commons.js`,
          `${CONFIG.DOMAIN}/edit/main.js`,
          ...commonsDebug,
        ],
  });
});
...
console.log(`编辑器：\x1B[35m${CONFIG.DOMAIN}/edit\x1B[0m`);
console.log(`页面预览：\x1B[35m${CONFIG.DOMAIN}/page\x1B[0m`);
console.log(
  `\x1B[32m----------↓ DEV模式 ↓(请勿退出进程,否则'npm run server'重启服务)--------------\x1B[0m`
);
console.log(`页面预览组件调试(XXX为组件文件夹名称，逗号隔开)：`);
console.log(
  `'npm run dev debug=XXX,YYY'  \x1B[35m${CONFIG.DOMAIN}/page?debug_comp=XXX,YYY\x1B[0m`
);
console.log(`编辑器预览组件调试：`);
console.log(
  `'npm run dev debug=XXX,YYY'  \x1B[35m${CONFIG.DOMAIN}/edit?debug_comp=XXX,YYY\x1B[0m`
);
console.log(`编辑器功能开发：`);
console.log(`'npm run dev:edit'  \x1B[35m${CONFIG.DOMAIN}/edit?debug=1\x1B[0m`);
