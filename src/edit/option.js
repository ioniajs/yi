  ...
  return (
    <div className={style.edt}>
      <a onClick={savePage}>
        <Tooltip
          {...TooltipProps}
          title={<span>保存({IsMacOS ? "Command" : "Ctrl"}+S)</span>}
        >
          <FolderTwoTone className={style.icons} />
        </Tooltip>
      </a>
      <a
        onClick={canReturnEdit ? returnEdit : undefined}
        className={fillUnable(canReturnEdit)}
      >
        <Tooltip
          {...TooltipProps}
          title={<span>撤销({IsMacOS ? "Command" : "Ctrl"}+Z)</span>}
        >
          <LeftCircleTwoTone className={style.icons} />
        </Tooltip>
      </a>
      <a
        onClick={canResumeEdit ? resumeEdit : undefined}
        className={fillUnable(canResumeEdit)}
      >
        <Tooltip
          {...TooltipProps}
          title={<span>恢复({IsMacOS ? "Command" : "Ctrl"}+Y)</span>}
        >
          <RightCircleTwoTone className={style.icons} />
        </Tooltip>
      </a>
      <a
        onClick={choose ? deleteNode : undefined}
        className={fillUnable(choose)}
      >
        <Tooltip
          {...TooltipProps}
          title={<span>删除({IsMacOS ? "删除键" : "DEL"})</span>}
        >
      ...
      <a onClick={choose ? copeNode : undefined} className={fillUnable(choose)}>
        <Tooltip
          {...TooltipProps}
          title={<span>复制({IsMacOS ? "Command" : "Ctrl"}+C)</span>}
        >
      ...
      <a onClick={choose ? cutNode : undefined} className={fillUnable(choose)}>
        <Tooltip
          {...TooltipProps}
          title={<span>剪切({IsMacOS ? "Command" : "Ctrl"}+X)</span>}
        >
          <PieChartTwoTone className={style.icons} />
        </Tooltip>
      </a>
      <a
        onClick={canPaste ? pasteNode : undefined}
        className={fillUnable(canPaste)}
      >
        <Tooltip
          {...TooltipProps}
          title={<span>粘贴({IsMacOS ? "Command" : "Ctrl"}+V)</span>}
        >
          <SnippetsTwoTone className={style.icons} />
        </Tooltip>
      </a>
      <a
        onClick={() => choose && changePosNode(-1)}
        className={fillUnable(choose)}
      >
      ...
      <a onClick={() => message.warn("请按住空格后拖动画布、鼠标滚轮")}>
        <Tooltip
          {...TooltipProps}
          title={<span>移动画布(Space+滚动+左键)</span>}
        >
...
const OptionBoard = ({ optionInputHasFocus }) => {
    ...
    ({ name, prop, type = "text", option, mirrorValue }, optionName) => {
      ...
      switch (type) {
        case "switch":
          ...
          return (
            <div className={style.longSwitch}>
              <p>{name}</p>
              <Switch
                checked={swtichValue}
                onChange={(checked) =>
                  changeValue({ checked, mirrorValue }, prop, type)
                }
              />
            </div>
          );
              ...
              <p>
                  ...
                  content={
                    <SketchPicker
                      ...
                      onChangeComplete={(color) =>
                        changeValue(color.rgb, prop, type)
                      }
                    />
                  }
                  trigger="click"
                >
                  <span className={style.colorBlock}>
                    <i style={{ backgroundColor: curValue }}></i>
                  </span>
                </Popover>
              </p>
              <Input
  ...
  const comp = useMemo(() => {
    ...
    const content = (
      <Layout className={style.tabPane}>
        {tabIndex === 0 ? (
          ...
          <p className={style.compName}>
            <span>
              {menu[chooseObj.current.name].name}({chooseObj.current.name})
            </span>
    ...
    return (
      <Tabs activeKey={tabIndex.toString()} onChange={changeTab}>
        {tab.map((tabText, i) => {
          if (!choose && i > 0) {
            return null;
          }
          return (
            <TabPane tab={tabText} key={`${i}`}>
              {content}
            </TabPane>
          );
        })}
  ...
  return (
    <>
      {/* <Edit /> */}
      <div className={style.opt}>{comp}</div>
    </>
  );
};
