...
const Board = () => {
  ...
  const repainting = useCallback((forceScale) => {
    ...
    if (
      ...
      const nextPaintScale = shouldFoceUpdate
        ? forceScale
        : (paintingWrapDom.offsetWidth - PaintBoxMargin * 2) / 1920;
      const nextPaintMinHeight =
        (paintingWrapDom.offsetHeight - PaintBoxMargin + 300) / nextPaintScale;
  ...
  const handlewheel = useCallback((e) => {
    ...
    if (spaceDown.current) {
      ...
      const scale = Math.min(
        ...
        Math.max(
          0.1,
          paintScaleRef.current - (e.deltaY > 0 ? scaleSpeed : -scaleSpeed)
        )
      );
  ...
  const cutNode = useCallback(() => {
    ...
    if (choose) {
      // 先拷贝剪切的对象再删除
      copyCompEl.current = JSON.parse(
        JSON.stringify(searchTree(tree, choose, EnumEdit.choose))
      );
  ...
  const pasteNode = useCallback(() => {
    ...
    if (typeof copyCompEl.current === "string") {
      copyObj = JSON.parse(
        JSON.stringify(searchTree(tree, copyCompEl.current, EnumEdit.choose))
      );
    } else {
      copyCompEl.current = null;
    }
    ...
    } else if (!menu[chooseObj.name].hasChild) {
      // 不允许插入的位置
  ...
  const handleClick = useCallback((el, e, expanded) => {
    ...
    // 需要展开侧边栏树
    if (expanded) {
      expandedParentsNode(targetCmpDom.current).forEach((ExpEl) =>
        expandedKeys.current.add(ExpEl)
      );
    }
  ...
  const changeBoxByMask = useCallback(
    (e) => {
      ...
      document.querySelector(`.${styleBd.topLeftTip}`).innerHTML = `${parseInt(
        computedStyle.left
      )},${parseInt(computedStyle.top)}`;
      document.querySelector(`.${styleBd.topTip}`).innerHTML = parseInt(
        computedStyle.width
      );
      document.querySelector(`.${styleBd.rightTip}`).innerHTML = parseInt(
  ...
  return (
    <Layout className={style.main}>
            ...
            >
              ...
              >
                ...
                <div
                  ...
                  onDragOver={(e) =>
                    handleEventCallBack("dragover", EnumId.root, null, e)
                  }
                  onDragLeave={(e) =>
                    handleEventCallBack("dragout", EnumId.root, null, e)
                  }
                  onDrop={(e) =>
                    handleEventCallBack("drop", EnumId.root, null, e)
                  }
      ...
      {ReactDom.createPortal(
        contextMenu && (
          <Menu id="contextMenu" style={contextMenu}>
            <Menu.Item key="0" onClick={deleteNode}>
              删除
            </Menu.Item>
            <Menu.Divider />
            <Menu.Item key="1" onClick={() => triggerShowEl(state.choose)}>
              隐藏
            </Menu.Item>
            <Menu.Divider />
            <Menu.Item key="2" onClick={copeNode}>
              复制
            </Menu.Item>
            <Menu.Divider />
            <Menu.Item key="3" onClick={cutNode}>
              剪切
            </Menu.Item>
            <Menu.Divider />
            <Menu.Item
